...
class AffrontementMulti(gym.Env):
    ...
    def reset(self, seed=None, options=None):
        ...
        self.pos_chasseurs = []
        self.traj_chasseurs = []
        for _ in range(self.nbr_chasseurs):
            self.pos_chasseurs.append(self.init_chasseur())
            self.traj_chasseurs.append([5, utils.comp_cap(self.pos_chasseur, self.pos_eviteur)])    
    ...
    def step(self, action_dict):
        ...
        if self.current_step >= self.max_step:
            ...
            if np.linalg.norm(self.pos_eviteur - self.pos_objectif) < 30:
                ...
                pen_pro = 5 if proche else 0
                terminated = {"eviteur": False, "chasseur1": False, "chasseur2": False}
                dist_evit_obj_new = np.linalg.norm(self.pos_eviteur - self.pos_objectif)
                rewards = {"eviteur": (dist_evit_obj_mem - dist_evit_obj_new)/10 - pen_pro, "chasseur1": 0, "chasseur2": 0}
